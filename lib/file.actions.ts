...
function extractTransactions(text: string): Transaction[] {
  ...
  const regex = /(\d{4}-\d{2}-\d{2})\s*-\s*KES\s(\d+.?\d)\sto\s([A-Za-z\s]+)\s*-\s*(.*)/g;
...
export async function processFile(
  ...
  try {
    ...
    filePath = path.join(uploadDir, `${Date.now()}-${file.name.replace(/\s+/g, "_")}`);
    const buffer = Buffer.from(await file.arrayBuffer());
    await fs.writeFile(filePath, buffer);

    // New parser usage – clean & reliable
    ...
    console.error("PDF processing failed:", error);  // ← improved logging
