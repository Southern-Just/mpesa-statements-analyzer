...
interface Props {
  onPreviewRequest: (filter: string) => void;
}

const SearchFilters = ({ onPreviewRequest }: Props) => {
      ...
      {isDate || isDateRange ? (
        <div className="flex overflow-hidden rounded-lg border border-gray-300 focus-within:border-gray-600">
          <div className="flex flex-1 border-r border-gray-900">
            ...
            {isDateRange && <input type="date" className={dateInputClass} />}
          </div>
